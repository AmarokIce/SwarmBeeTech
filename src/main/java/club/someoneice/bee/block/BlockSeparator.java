...
public class BlockSeparator extends BlockContainer {
    ...
    public BlockSeparator() {
        super(Material.iron);
        this.setBlockName("bee_separate");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        ...
        if (!(tileEntity instanceof TileSeparator)) return false;
        TileSeparator tile = (TileSeparator) tileEntity;
        if (!world.isRemote) {
            ((EntityPlayerMP) player).playerNetServerHandler.sendPacket(tile.getDescriptionPacket());
            player.openGui(BeeTech.INSTANCE, 2, world, x, y, z);
        }
        return true;
    }
    ...
    public void registerBlockIcons(IIconRegister register) {
        for(int i = 0; i < 3; ++i)
            this.icons[i] = register.registerIcon(MMM.getTextureName("bee_separate_" + (i + 1)));
    }
