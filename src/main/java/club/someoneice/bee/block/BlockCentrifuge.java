...
public class BlockCentrifuge extends BlockContainer {
    ...
    public BlockCentrifuge() {
        super(Material.wood);
        this.setBlockName("centrifuge");
        this.setCreativeTab(ManaMetalMod.tab_beekeep);

        GameRegistry.registerBlock(this, "centrifuge");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        ...
        if (!(tileEntity instanceof TileCentrifuge)) return false;
        TileCentrifuge tile = (TileCentrifuge) tileEntity;
        if (!world.isRemote) {
            ((EntityPlayerMP) player).playerNetServerHandler.sendPacket(tile.getDescriptionPacket());
            player.openGui(BeeTech.INSTANCE, 1, world, x, y, z);
        }
        return true;
    }
    ...
    public void registerBlockIcons(IIconRegister register) {
        for(int i = 0; i < 3; ++i)
            this.icons[i] = register.registerIcon(BeeTech.MODID + ":centrifuge_" + (i + 1));
    }
