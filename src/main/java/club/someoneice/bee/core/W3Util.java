...
public class W3Util {
    /* From Wheat-field With Wolf */
    /* Maybe I should make it in a lib... I use this everywhere! */
    ...
    public boolean checkArraySizeSame(Object[] A, Object[] B) {
        List<Object> objectsA = ObjectUtil.objectRun(Lists.newArrayList(), it -> {
            Arrays.stream(A).forEach(item -> {
                if (item == null) return;
                it.add(item);
            });
        });
        List<Object> objectsB = ObjectUtil.objectRun(Lists.newArrayList(), it -> {
